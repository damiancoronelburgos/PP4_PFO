...
export default function Preceptor() {
  ...
  const savePassword = (e) => {
    ...
    setShowPwd(false); setPwd1(""); setPwd2("");
  };
  ...
  const dateOptions = useMemo(() => {
    ...
    if (!uniq.find(o => o.value === todayISO)) uniq.unshift({ value: todayISO, label: `${fmtFecha(todayISO)} (hoy)` });
  ...
  // Alumnos por comisión (desde calificaciones)
  ...
  // Conteo para badge de pendientes (considera borradores)
  ...
  // Próximos 21 días (para panel “Próximos eventos”)
  const proximosEventos = useMemo(() => {
    const start = todayISO;
    const endDate = new Date(todayISO); endDate.setDate(endDate.getDate() + 21);
  ...
  const addComision    = (id) => { if (!id) return; setCommsComs((p) => (p.includes(id) ? p : [...p, id])); setCommsComSel(""); };
            ...
            <div className="enroll-card">
              ...
              <div className="grades-table-wrap">
                <table className="grades-table w-full">
                  ...
                  <tbody>
                    {CLASES_DE_HOY.map(c => (
                      <tr key={c.id}><td>{c.materia}</td><td>{c.comision}</td><td>{c.horario}</td><td>{c.aula}</td></tr>
                    ))}
                  </tbody>
                </table>
              </div>
            ...
            <div className="grid-gap">
              ...
              <div className="enroll-card">
                ...
                {proximosEventos.length === 0 ? (
                  <div className="muted"><p>No hay eventos en las próximas 3 semanas.</p></div>
                ) : (
                  <div className="grades-table-wrap">
                    <table className="grades-table w-full">
                      <thead><tr><th>Fecha</th><th>Título</th><th>Comisión</th></tr></thead>
                      <tbody>
                        {proximosEventos.map(ev => (
                          <tr key={ev.id}><td>{fmtFecha(ev.fecha)}</td><td>{ev.titulo}</td><td>{ev.comision}</td></tr>
                        ))}
                      </tbody>
                    </table>
                  </div>
  ...
  const renderJustificaciones = () => {
    ...
    const rows = (justifDb || [])
      ...
      .filter(j => {
        ...
        const ape = norm(a.apellido || ""), nom = norm(a.nombre || "");
        const nombreA = `${ape}, ${nom}`, nombreB = `${nom} ${ape}`;
      ...
      <div className="notes-card">
        ...
        <div>
          {notisVisibles.map((n) => (
            <div key={n.id} className={"note-item" + (n.leida ? "" : " unread")}>
              <div className="note-head">
                <button className="note-fav-btn" title={n.fav ? "Quitar de favoritos" : "Marcar como favorito"} onClick={() => toggleFav(n.id)}>
                  {n.fav ? "★" : "☆"}
                </button>
          ...
          {notisVisibles.length === 0 && (
            <div className="note-item"><div className="note-detail">No hay notificaciones para mostrar.</div></div>
          )}
        </div>
      </div>
    </div>
